...
use cim_domain_identity::{
    // Components
    IdentityId, IdentityEntity, IdentityType, IdentityStatus,
    IdentityVerification, VerificationLevel,
    IdentityRelationship, RelationshipType,
    
    // Commands
    CreateIdentityCommand, UpdateIdentityCommand,
    EstablishRelationshipCommand, MergeIdentitiesCommand,
    
    // Events
    IdentityCreated, IdentityUpdated, 
    RelationshipEstablished, IdentitiesMerged,
    
    // Systems
    create_identity_system, update_identity_system,
    establish_relationship_system, merge_identities_system,
...
fn test_identity_lifecycle() {
    ...
    // Send command
    world.resource_mut::<Events<CreateIdentityCommand>>().send(create_cmd);
    ...
    let identities: Vec<_> = world.query::<&IdentityEntity>()
        .iter(&world)
        .collect();
...
fn test_identity_update() {
    ...
    world.resource_mut::<Events<UpdateIdentityCommand>>().send(update_cmd);
    ...
    // Verify update
    let identity = world.query::<&IdentityEntity>()
        .iter(&world)
...
fn test_relationship_establishment() {
    ...
    world.resource_mut::<Events<EstablishRelationshipCommand>>().send(establish_cmd);
    ...
    // Verify relationship
    let relationships: Vec<_> = world.query::<&IdentityRelationship>()
        .iter(&world)
        .collect();
...
fn test_identity_merge() {
    ...
    world.resource_mut::<Events<MergeIdentitiesCommand>>().send(merge_cmd);
    ...
    let source = world.query::<&IdentityEntity>()
        .iter(&world)
        .find(|e| e.identity_id == source_id)
        .unwrap();
    
    assert!(matches!(source.status, IdentityStatus::Merged { merged_into } if merged_into == target_id));
}
