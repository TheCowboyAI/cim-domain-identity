...
use cim_domain_identity::{
    PersonEvent, PersonId,
    OrganizationEvent, OrganizationId, OrganizationType,
};
use cim_domain_identity::domain::{Email, Name, PhoneNumber, Address, TrustLevel, AuthMethod, MfaMethod};
use chrono::Utc;
...
mod person_event_tests {
    ...
    fn test_person_registered_event() {
        ...
        match event {
            PersonEvent::PersonRegistered { person_id: id, name: n, email: e } => {
    ...
    fn test_authentication_events() {
        ...
        match success_event {
            PersonEvent::AuthenticationSucceeded { person_id: pid, method, timestamp: ts } => {
    ...
    fn test_trust_level_events() {
        ...
        match event {
            PersonEvent::TrustLevelChanged { old_level: old, new_level: new, .. } => {
    ...
    fn test_organization_membership_events() {
        ...
        match joined_event {
            PersonEvent::JoinedOrganization { person_id: pid, organization_id: oid } => {
                assert_eq!(pid, person_id);
        ...
        match left_event {
            PersonEvent::LeftOrganization { person_id: pid, organization_id: oid } => {
                assert_eq!(pid, person_id);
                assert_eq!(oid, org_id);
            }
...
mod organization_event_tests {
    ...
    fn test_organization_created_event() {
        ...
        // Then: Event contains correct data
        match event {
            OrganizationEvent::OrganizationCreated { organization_id, name: n, org_type: t } => {
    ...
    fn test_member_added_event() {
        ...
        match event {
            OrganizationEvent::MemberAdded { organization_id, person_id: pid } => {
                assert_eq!(organization_id, org_id);
                assert_eq!(pid, person_id);
            }
    ...
    fn test_member_removed_event() {
        ...
        match event {
            OrganizationEvent::MemberRemoved { organization_id: oid, person_id: pid } => {
                assert_eq!(oid, org_id);
                assert_eq!(pid, person_id);
            }
    ...
    fn test_organization_hierarchy_events() {
        ...
        match parent_event {
            OrganizationEvent::ParentChanged { old_parent_id, new_parent_id, .. } => {
        ...
        match child_event {
            OrganizationEvent::ChildAdded { organization_id: oid, child_id: cid } => {
                assert_eq!(oid, org_id);
